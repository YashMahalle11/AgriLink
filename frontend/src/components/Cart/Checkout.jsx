                        ...
                        {!checkoutId ? (
                            ...
                            <div>
                                ...
                                <RazorpayButton
                                    ...
                                    onError={(err) => alert("Razorpay payment failed. Try again.")}
                                />
                            </div>
                        )}
                    </div>
...
/*import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import PayPalButton from './RazorpayButton';
import RazorpayButton from './RazorpayButton'; // ✅ Razorpay Button
import { useDispatch, useSelector } from 'react-redux';
import { createCheckout } from "../../redux/slices/checkoutSlice";
import axios from 'axios';
...
const Checkout = () => {
    ...
    const [checkoutId, setCheckoutId] = useState(null);
    const [paymentMethod, setPaymentMethod] = useState("Razorpay"); // ✅ Default method
    const [shippingAddress, setShippingAddress] = useState({
        firstName: "",
        lastName: "",
        address: "",
 ...
 /*                   {!checkoutId && (
                    ...
                    <div className='mt-6'>
                        {!checkoutId ? (
                            ...
                            <div>
                                <h3 className='text-lg mb-4'>Pay with {paymentMethod}</h3>
...
useEffect(()=> {
    ...
    const handleCreateCheckout = async (e) => {
        ...
        if(cart && cart.products.length > 0){
             ...
             if (res.payload && res.payload._id){
                setCheckoutId(res.payload._id); // Set cjeckout ID if checkout was successful
             }
        }
    };
